...
class Command(object):
    ...
    def run(self):
        ...
        print("CHECK self.query ", self.query, type(self.query))
        if self.stream:
            ...
            for line in self.process.stdout:
                self.callback(line.decode(
                    self.encoding, 'replace').replace('\r', ''))
                hasWritten = True
        ...
        results, errors = self.process.communicate(
            input=self.query.encode(self.encoding))
        print("RESULTS ARE ", results, type(results))
        ...
        print("resultString ARE ", resultString, type(resultString))
...
class ThreadCommand(Command, Thread):
    ...
    def stop(self):
        ...
        try:
            ...
            logger.info(
                "command execution exceeded timeout (%s s), process killed", self.timeout)
            self.callback(("Command execution time exceeded 'thread_timeout' ({0} s).\n"
                           "Process killed!\n\n"
                           ).format(self.timeout))
        except Exception:
            logger.info(
                "command execution exceeded timeout (%s s), process could not be killed", self.timeout)
            self.callback(("Command execution time exceeded 'thread_timeout' ({0} s).\n"
