...
You might need to restart the editor for settings to be refreshed."""
    ...
    def getTables(self, callback):
        if self.cli == 'impala':
            callback(self.impala_conn.get_tables())
        else:
    ...
    def getTableRecords(self, tableName, callback):
        ...
        self.runFormattedNamedQueryCommand(
            'show records', formatValues, callback)
    ...
    def getFunctionDescription(self, functionName, callback):
        self.runFormattedNamedQueryCommand(
            'desc function', functionName, callback)
    ...
    def execute(self, queries, callback, stream=None, check=None):
        queryName = 'execute'
        ...
        if check:
            ...
            print("callback is ", callback, type(callback))
            print("queryToRun is ", queryToRun, type(queryToRun))
            print("stream is ", stream, type(stream))
            print("DONE")
    ...
    def buildNamedQuery(self, queryName, queries):
        ...
        beforeQuery = cliOptions.get('queries', {}).get(
            queryName, {}).get('before')
        afterQuery = cliOptions.get('queries', {}).get(
            queryName, {}).get('after')
    ...
    def buildArgs(self, queryName=None):
        ...
        if optionalArgs:  # only if we have optional args
            if isinstance(optionalArgs, list):
                for item in optionalArgs:
                    formattedItem = self.formatOptionalArgument(
                        item, self.options)
                    if formattedItem:
                        args = args + shlex.split(formattedItem)
            else:
                formattedItem = self.formatOptionalArgument(
                    optionalArgs, self.options)
                if formattedItem:
                    args = args + shlex.split(formattedItem)
        ...
        if queryName:
            queryOptions = cliOptions.get('queries', {}).get(
                queryName, {}).get('options')
            if queryOptions:
        ...
        if optionalEnv:  # only if we have optional args
            if isinstance(optionalEnv, dict):
                for var, value in optionalEnv.items():
                    formattedValue = self.formatOptionalArgument(
                        value, self.options)
                    if formattedValue:
