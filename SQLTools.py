...
def startPlugin():
    ...
    SETTINGS_FILENAME_DEFAULT = os.path.join(
        DEFAULT_FOLDER, SQLTOOLS_SETTINGS_FILE)
    CONNECTIONS_FILENAME = os.path.join(USER_FOLDER, SQLTOOLS_CONNECTIONS_FILE)
    CONNECTIONS_FILENAME_DEFAULT = os.path.join(
        DEFAULT_FOLDER, SQLTOOLS_CONNECTIONS_FILE)
    QUERIES_FILENAME = os.path.join(USER_FOLDER, SQLTOOLS_QUERIES_FILE)
    QUERIES_FILENAME_DEFAULT = os.path.join(
        DEFAULT_FOLDER, SQLTOOLS_QUERIES_FILE)

    try:
        settingsStore = Settings(
            SETTINGS_FILENAME, default=SETTINGS_FILENAME_DEFAULT)
    except Exception as e:
        msg = '{0}: Failed to parse {1} file'.format(
            __package__, SQLTOOLS_SETTINGS_FILE)
        logging.exception(msg)
        Window().status_message(msg)

    try:
        connectionsStore = Settings(
            CONNECTIONS_FILENAME, default=CONNECTIONS_FILENAME_DEFAULT)
    except Exception as e:
        msg = '{0}: Failed to parse {1} file'.format(
            __package__, SQLTOOLS_CONNECTIONS_FILE)
        logging.exception(msg)
        Window().status_message(msg)
...
class ST(EventListener):
    ...
    def setDefaultConnection():
        ...
        if default not in ST.connectionDict:
            logger.error(
                'connection "%s" set as default, but it does not exists', default)
            return
    ...
    def setConnection(connectionName, callback=None):
        ...
        def createConnection(connectionName, config, settings, callback=None):
            ...
            try:
                ...
                Window().status_message(__package__ +
                                        ": " + str(e).splitlines()[0])
                raise e
    ...
    def loadConnectionData(callback=None):
        ...
        def afterAllDataHasLoaded():
            ST.completion = Completion(
                ST.tables, ST.columns, ST.functions, settings=settingsStore)
            logger.info('completions loaded')
            # print("tables are ", ST.tables)
            # print("columns are ", ST.columns)
            # print("functions are ", ST.functions)
            if (callback):
                callback()
        ...
        print("SUCCESSFULLY GET TABLES ", ST.tables)
    ...
    def selectConnectionQuickPanel(callback=None):
        ...
        def connectionMenuList(connDictionary):
            ...
            for name, config in ST.connectionDict.items():
                ...
                user = '{}@'.format(config.get('username', '')
                                    ) if 'username' in config else ''
                # user = config.get('username', '')
                host = config.get('host', '')
                port = ':{}'.format(config.get('port', '')
                                    ) if 'port' in config else ''
                db = '/{}'.format(config.get('database', '')
                                  ) if 'database' in config else ''
    ...
    def on_query_completions(view, prefix, locations):
        ...
        # get completions
        autoCompleteList, inhibit = ST.completion.getAutoCompleteList(
            prefix, sql, sqlToCursor)
...
class StShowRecords(WindowCommand):
    ...
    def run():
        if not ST.conn:
            ST.selectConnectionQuickPanel(
                callback=lambda: Window().run_command('st_show_records'))
            return
...
class StDescTable(WindowCommand):
    ...
    def run():
        ...
        if not ST.conn:
            ST.selectConnectionQuickPanel(
                callback=lambda: Window().run_command('st_desc_table'))
            return
...
class StDescFunction(WindowCommand):
    ...
    def run():
        ...
        if not ST.conn:
            ST.selectConnectionQuickPanel(
                callback=lambda: Window().run_command('st_desc_function'))
            return
...
class StExplainPlan(WindowCommand):
    ...
    def run():
        if not ST.conn:
            ST.selectConnectionQuickPanel(
                callback=lambda: Window().run_command('st_explain_plan'))
            return
...
class StExecute(WindowCommand):
    ...
    def run():
        if not ST.conn:
            ST.selectConnectionQuickPanel(
                callback=lambda: Window().run_command('st_execute'))
            return

        Window().status_message(MESSAGE_RUNNING_CMD)
        ST.conn.execute(getSelectionText(), createOutput(), check=True)
...
class StExecuteAll(WindowCommand):
    ...
    def run():
        if not ST.conn:
            ST.selectConnectionQuickPanel(
                callback=lambda: Window().run_command('st_execute_all'))
            return
...
class StFormat(TextCommand):
    ...
    def run(edit):
        ...
        for region in selectionRegions:
            ...
            View().replace(edit, region, Utils.formatSql(
                textToFormat, settingsStore.get('format', {})))
...
class StVersion(WindowCommand):
    ...
    def run():
        sublime.message_dialog(
            'Using {0} {1}'.format(__package__, __version__))
...
class StHistory(WindowCommand):
    ...
    def run():
        if not ST.conn:
            ST.selectConnectionQuickPanel(
                callback=lambda: Window().run_command('st_history'))
            return
